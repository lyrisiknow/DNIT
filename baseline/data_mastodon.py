...
if __name__ == '__main__':
    ...
    result_record("TWIND", calculate_binary_auc(IG, G), "mastodon", param='auc')
    
    print('TENDS:')
    IG = nx.DiGraph()
    IG.add_nodes_from(nodes)
    IG.add_edges_from(tends_algorithm(N, S))
    result_record("TENDS", calculate_binary_auc(IG, G), "mastodon", param='auc')
    
    print('SIDN:')
    IG = nx.DiGraph()
    IG = nx.from_numpy_array(infer_sidn_network(S), create_using=nx.DiGraph)
    result_record("SIDN", calculate_binary_auc(IG, G), "mastodon", param='auc')
    
    print('PIND:')
    IG = nx.DiGraph()
    IG = nx.from_numpy_array(pind_inference(S), create_using=nx.DiGraph)
    result_record("PIND", calculate_binary_auc(IG, G), "mastodon", param='auc')