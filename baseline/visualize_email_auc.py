        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                for key, values in record.items():
                    # 匹配 email_模型_auc 格式
                    if key.startswith("email_") and key.endswith("_auc"):
...
# 替换代码中的这两行
        ...
        ax.set_title('Model Performance Comparison (Email Dataset)')
        ax.set_xticks(x)
        ...
        # 保存并显示
        plt.savefig('email_model_comparison.png', dpi=300)
        plt.show()
        print("图表已保存为: email_model_comparison.png")
