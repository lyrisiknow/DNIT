        ...
        try:
            ...
            for key, val in obj.items():
                ...
                if match:
                    ...
                    # 只处理在样式文件中存在的模型
                    if model_name in model_styles and 100 <= n_val <= 300:
...
for ax, title in zip([ax1, ax2], ['ROC-AUC score', 'PR-AUC score']):
    ...
    # ================= 修改部分 =================
    # 将原来的 labelsize=12 修改为 labelsize=fn - 2 (即 14)
...
plt.savefig('roc_pr_auc_n100.png', dpi=300, bbox_inches='tight', pad_inches=0.05)
plt.savefig('roc_pr_auc_n100.pdf', dpi=300, bbox_inches='tight', pad_inches=0.05)
plt.show()